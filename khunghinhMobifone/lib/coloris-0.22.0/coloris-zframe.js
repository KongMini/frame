...
((window, document, Math, undefined) => {
        ...
        case 'swatches':
          if (Array.isArray(options.swatches)) {
            ...
            options.swatches.forEach((swatch, i) => {
              swatches.push(
                `<button type="button" id="clr-swatch-${i}" aria-labelledby="clr-swatch-label clr-swatch-${i}" style="color: ${swatch};">${swatch}</button>`
              );
            });

            getEl('clr-swatches').innerHTML = swatches.length
              ? `<div>${swatches.join('')}</div>`
              : '';
  ...
  function setColor(selector, newColor) {
    if (typeof selector === 'string' && typeof newColor === 'string') {
      // instances[selector]
      // console.log(instances[selector]);
      const currentElement = document.querySelector(selector);
      // console.log(currentElement);
      if (currentElement) {
        currentElement.value = newColor;
  ...
  function open(selector) {
    ...
    // Apply any per-instance options first
    attachVirtualInstance(currentElement);
    ...
    if (!settings.inline) {
      ...
      if (parent) {
        ...
        if (left + pickerWidth > document.documentElement.clientWidth) {
          left += coords.width - pickerWidth;
          if (left < 0) {
            left = (document.documentElement.clientWidth - pickerWidth) / 2;
          }
          reposition.left = true;
        }
  ...
  function init() {
    ...
    addListener(clearButton, 'click', (event) => {
      pickColor('#000000');
      if (settings.onClear) {
        settings.onClear.call(window, colorValue.value);
      }
    ...
    addListener(closeButton, 'click', (event) => {
      if (settings.onClose) {
        settings.onClose.call(window, colorValue.value);
      }
